...
func TestThuanTuyenXepTrenNghichTuyen(t *testing.T) {
	...
	ranked := app.RankBidsForAsk(truck, []entity.Bid{nghichTuyen, thuanTuyen}, app.DefaultScoreWeights())
	if len(ranked) == 0 {
...
func TestDonGanXeXepTrenDonXa(t *testing.T) {
	...
	ranked := app.RankBidsForAsk(truck, []entity.Bid{xa, gan}, app.DefaultScoreWeights())
	if len(ranked) == 0 {
		t.Fatal("không có đơn nào qua ngưỡng")
	}
...
func TestDonVuotTaiTrongBiLoai(t *testing.T) {
	...
	ranked := app.RankBidsForAsk(truck, []entity.Bid{quaNang, quaTo, vua}, app.DefaultScoreWeights())
	if len(ranked) != 1 {
...
func TestGiaThapHonMucToiThieuBiLoai(t *testing.T) {
	...
	if ranked := app.RankBidsForAsk(truck, []entity.Bid{reQua}, app.DefaultScoreWeights()); len(ranked) != 0 {
...
func TestDonLapDayNhieuHonDuocUuTien(t *testing.T) {
	...
	ranked := app.RankBidsForAsk(truck, []entity.Bid{itHang, nhieuHang}, app.DefaultScoreWeights())
	if len(ranked) < 2 {
...
func TestDiemNamTrongKhoangHopLe(t *testing.T) {
	truck := truckSaigonToHanoi()
	ranked := app.RankBidsForAsk(truck,
		[]entity.Bid{cargo(danang, hanoi, 3000, 12, 5_000_000)},
		app.DefaultScoreWeights())
...
func TestKetQuaSapXepGiamDan(t *testing.T) {
	...
	ranked := app.RankBidsForAsk(truck, bids, app.DefaultScoreWeights())
...
func TestXeTimHangChoChuyenVe(t *testing.T) {
	...
	ranked := app.RankBidsForAsk(xeDangOHanoi,
		[]entity.Bid{hangDiNguoc, hangVeNam}, app.DefaultScoreWeights())
	if len(ranked) == 0 {
		t.Fatal("không có đơn nào qua ngưỡng")
	}
