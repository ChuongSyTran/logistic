...
func StartOfferConsumer(
	ctx context.Context,
	consumer app.EventConsumer,
	engine app.MatchingEngine,
	...
	handler := func(ctx context.Context, subject string, payload []byte) error {
		...
		if err := proto.Unmarshal(payload, &pbAsk); err != nil {
			return fmt.Errorf("%w: giải mã Ask thất bại: %v", entity.ErrNonRetryable, err)
		}

		ask, err := appMapper.PbAskToEntity(&pbAsk)
		if err != nil {
			return fmt.Errorf("%w: dựng entity Ask thất bại: %v", entity.ErrNonRetryable, err)
		}
		...
		if ask.ID, err = mapper.BytesToUUID(pbAsk.GetId()); err != nil {
			return fmt.Errorf("%w: askID trong payload không hợp lệ: %v", entity.ErrNonRetryable, err)
		}
