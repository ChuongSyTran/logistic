...
func InitSubcriber(natJetStreamContext nats.JetStreamContext) app.EventConsumer {
	return &natsSubcriber{
		natJetStreamContext: natJetStreamContext,
	}
}

var _ app.EventConsumer = (*natsSubcriber)(nil)

func (n *natsSubcriber) Consume(ctx context.Context, topic string, handler app.EventHandler) error {
	...
	_, err := n.natJetStreamContext.QueueSubscribe(topic, name, func(msg *nats.Msg) {
		...
		if err != nil {
			if errors.Is(err, entity.ErrNonRetryable) {
				log.Printf("Discarding poison pill message: %v", err)
				msg.Ack()
				return
			}

			if errors.Is(err, entity.ErrRetryWithDelay) {
