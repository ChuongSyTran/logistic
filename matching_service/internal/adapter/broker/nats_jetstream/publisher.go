...
import (
	...
	"fmt"
	"log"
	"matching_service/internal/adapter/broker"
...
var _ app.EventPublisher = (*natsPublisher)(nil)
...
func InitPublisher(natJetStreamContext nats.JetStreamContext, appMapper mapper.MatchingMapper) app.EventPublisher {
...
func (n *natsPublisher) Publish(ctx context.Context, msg *app.EventMessage) error {
	if msg == nil {
...
func parseNatsError(err error) error {
	if errors.Is(err, nats.ErrTimeout) ||
		...
		return fmt.Errorf("%w: %v", entity.ErrRetryWithDelay, err)
	}
	...
	if errors.Is(err, nats.ErrMaxPayload) ||
		...
		return fmt.Errorf("%w: %v", entity.ErrNonRetryable, err)
	}

	if apiErr, ok := errors.AsType[*nats.APIError](err); ok {
		log.Printf("NATS JetStream API Error, do not retry: %v", apiErr)
		return fmt.Errorf("%w: %v", entity.ErrNonRetryable, apiErr)
	}

	return fmt.Errorf("%w: unknown NATS error: %v", entity.ErrRetryWithDelay, err)
}
