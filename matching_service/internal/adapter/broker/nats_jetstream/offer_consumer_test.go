...
func (f *fakeConsumer) Consume(_ context.Context, topic string, handler app.EventHandler) error {
	f.topic = topic
	f.handler = handler
	return f.err
}
...
func startWith(t *testing.T, engine app.MatchingEngine) *fakeConsumer {
	t.Helper()
