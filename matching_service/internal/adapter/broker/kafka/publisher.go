...
var _ app.EventPublisher = (*kafkaPublisher)(nil)

func NewKafkaPublisher(brokers []string, appMapper mapper.MatchingMapper) (app.EventPublisher, error) {
...
func (p *kafkaPublisher) Publish(ctx context.Context, msg *app.EventMessage) error {
	if msg == nil {
	...
	if err != nil {
		if kerr, ok := errors.AsType[sarama.KError](err); ok {
			...
			case sarama.ErrRequestTimedOut,
				...
				return fmt.Errorf("%w: %v", entity.ErrRetryWithDelay, kerr)
			...
			case sarama.ErrMessageSizeTooLarge,
				...
				return fmt.Errorf("%w: %v", entity.ErrNonRetryable, kerr)
			}
		}

		log.Printf("Unknown Kafka error, returning 503: %v", err)
		return fmt.Errorf("%w: %v", entity.ErrServiceUnavailable503, err)
	}
